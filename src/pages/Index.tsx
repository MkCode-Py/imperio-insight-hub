...
import { getAllPosts, getFeaturedPosts, getMostViewedPosts, getAllCategories, formatViews } from '@/lib/blog';
...
const Index = () => {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main>
        {/* Hero */}
        <section className="container pt-10 pb-14 sm:pt-14 sm:pb-20">
          <div className="max-w-xl mx-auto text-center mb-10">
            <p className="section-label mb-3">Império Pharma · Editorial</p>
            <h1 className="text-3xl sm:text-4xl lg:text-[2.75rem] font-extrabold text-foreground leading-[1.15] text-balance">
              Ciência, Performance<br className="hidden sm:block" /> & Saúde
            </h1>
            <p className="text-muted-foreground mt-4 text-[15px] sm:text-base leading-relaxed max-w-md mx-auto">
              Conteúdo educacional de alta qualidade sobre ergogênicos, peptídeos, emagrecimento e protocolos de ciclos.
            </p>
          </div>

          <SearchBar className="max-w-lg mx-auto mb-12" />
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
              {categories.map((cat) => (
                <Link
                  ...
                  className="group relative rounded-lg overflow-hidden aspect-[4/3] imperio-card-hover"
                >
                  <img
                    src={cat.coverImage}
                    alt={cat.name}
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-3.5 sm:p-4">
                    ...
                    <p className="text-white/50 text-xs mt-0.5 line-clamp-1 hidden sm:block">{cat.description}</p>
                  </div>
                </Link>
              ))}
            </div>
        ...
        <section className="py-14 border-t border-border/40">
          <div className="container">
            ...
            <div className="flex items-center justify-between mb-7">
              <h2 className="text-xl sm:text-2xl font-bold text-foreground">Recentes</h2>
        ...
        {/* Most Viewed + Guides */}
        <section className="py-14 border-t border-border/40 bg-muted/30">
          <div className="container">
            <div className="grid grid-cols-1 lg:grid-cols-5 gap-10">
                ...
                <div className="space-y-2">
                  {mostViewed.map((post, i) => (
                    <Link
                      key={post.id}
                      to={`/post/${post.slug}`}
                      className="group flex items-center gap-4 p-3.5 rounded-lg bg-card border border-border/50 imperio-card-hover"
                    >
                      <span className="text-2xl font-extrabold text-accent/20 w-7 text-center shrink-0 tabular-nums">{i + 1}</span>
              ...
              <div className="lg:col-span-2">
                ...
                <div className="flex items-center justify-between mb-6">
                  <h2 className="text-xl sm:text-2xl font-bold text-foreground flex items-center gap-2">
                    <BookOpen className="h-5 w-5 text-accent" /> Guias
                  </h2>
                  <Link to="/guias" className="text-sm font-medium text-accent hover:underline flex items-center gap-1">
                    Ver todos <ArrowRight className="h-3.5 w-3.5" />
                  </Link>
                </div>
                <div className="divide-y divide-border/50 bg-card rounded-lg border border-border/50 px-3">
        ...
        <section className="py-16 border-t border-border/40">
          <div className="container">
            <div className="relative rounded-xl overflow-hidden p-8 sm:p-12 imperio-gradient text-primary-foreground text-center">
              <div className="relative z-10 max-w-md mx-auto">
                <h2 className="text-2xl sm:text-3xl font-bold mb-3">
                  Visite a Loja Império Pharma
                </h2>
                <p className="text-primary-foreground/70 mb-6 text-sm sm:text-base leading-relaxed">
                  Encontre os melhores produtos com qualidade garantida e entrega em todo o Paraguai.
                </p>
                <a
                  href="#loja"
                  className="inline-flex items-center gap-2 px-6 py-2.5 rounded-md bg-white text-primary font-semibold text-sm hover:bg-white/90 transition-colors"
                >
