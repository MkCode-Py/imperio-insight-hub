    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="container -mt-14 sm:-mt-16 relative z-10">
        <div className="max-w-3xl mx-auto">
          ...
          <div className="mt-5 mb-10">
            ...
            <div className="flex flex-wrap items-center gap-3 mt-6 text-sm text-muted-foreground">
              ...
              <span className="flex items-center gap-1.5"><Calendar className="h-3 w-3" />{formatDate(post.publishDate)}</span>
              <span className="flex items-center gap-1.5"><Clock className="h-3 w-3" />{post.readingTime} min</span>
              <span className="flex items-center gap-1.5"><Eye className="h-3 w-3" />{formatViews(post.uniqueViews + localViews)}</span>
            </div>
              ...
              {post.tags.slice(0, 3).map((tagId) => {
                ...
                return (
                  <Link key={tag.id} to={`/tag/${tag.slug}`}>
                    <Badge variant="outline" className="text-[11px] border-border/60 hover:bg-accent/8 hover:text-accent transition-colors">
                      {tag.name}
                    </Badge>
                  </Link>
                );
        ...
        <div className="max-w-3xl mx-auto mt-14 flex items-stretch gap-3">
          {prevPost ? (
            <Link to={`/post/${prevPost.slug}`} className="flex-1 group p-3.5 rounded-lg border border-border/40 hover:border-accent/30 transition-colors bg-card">
              <span className="text-[10px] text-muted-foreground flex items-center gap-1 mb-1.5 uppercase tracking-wider font-medium"><ArrowLeft className="h-3 w-3" /> Anterior</span>
          ...
          {nextPost ? (
            <Link to={`/post/${nextPost.slug}`} className="flex-1 group p-3.5 rounded-lg border border-border/40 hover:border-accent/30 transition-colors bg-card text-right">
              <span className="text-[10px] text-muted-foreground flex items-center gap-1 justify-end mb-1.5 uppercase tracking-wider font-medium">Próximo <ArrowRight className="h-3 w-3" /></span>
