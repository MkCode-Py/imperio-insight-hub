...
const Categories = () => {
  ...
  const handleSave = () => {
    ...
    if (editingCategory) {
      ...
      const newCat: Category = {
        ...
        coverImage: formImage || 'https://images.unsplash.com/photo-1534438327276-14e5300c3a48?w=800&q=80',
  ...
  const moveUp = (i: number) => {
    ...
    setCategories((prev) => {
      ...
      return arr.map((c, idx) => ({ ...c, order: idx + 1 }));
    });
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Button size="sm" className="gap-2" onClick={openNew}><Plus className="h-4 w-4" /> Nova Categoria</Button>
      </div>
        ...
        {categories.map((cat, i) => (
          <Card key={cat.id} className="border-border">
            <CardContent className="p-4 flex items-center gap-4">
              ...
              <img src={cat.coverImage} alt={cat.name} className="w-14 h-14 rounded-lg object-cover shrink-0" />
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <p className="font-medium text-foreground">{cat.name}</p>
                  {cat.highlighted && <Badge variant="outline" className="text-[10px]">Destaque</Badge>}
                  <Badge variant={cat.enabled ? 'default' : 'secondary'} className="text-[10px]">{cat.enabled ? 'Ativo' : 'Inativo'}</Badge>
                </div>
                <p className="text-xs text-muted-foreground truncate mt-0.5">/{cat.slug} · {cat.description}</p>
              </div>
